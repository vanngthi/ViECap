...
def magic_search_altclip(
    ...
    """
    Magic Search adapted for AltCLIP (BAAI/AltCLIP-m18).
    Image-guided decoding for caption generation.
    """
    ...
    if embeddings is not None:
        ...
        if tokens is None:
            tokens = torch.tensor(tokenizer.encode(prompts)).unsqueeze(0).to(device)
        generating = model.transformer.wte(tokens)
    generating = generating.float()

    # --- prepare AltCLIP model ---
    if clip_model is None or clip_processor is None:
        from transformers import AutoModel, AutoProcessor
        clip_model_id = "BAAI/AltCLIP-m18"
    ...
    if images_feature is None:
        ...
        with torch.no_grad():
            images_feature = clip_model.get_image_features(**image_inputs)
            images_feature = F.normalize(images_feature, dim=-1)
    else:
        images_feature = F.normalize(images_feature.float().to(device), dim=-1)
    ...
    for step in range(max_len):
        ...
        if step == 0:
            ...
            past_hidden_states = outputs.hidden_states[-1]

        # Top-k candidates
        b, n_seq, lm_hidden_size = past_hidden_states.size()
        next_token_probs = F.softmax(next_token_logits, dim=-1)
        _, top_k_indices = torch.topk(next_token_logits, dim=-1, k=top_k)
        top_k_probs = torch.gather(next_token_probs, dim=1, index=top_k_indices)

        # --- compute image–text similarity using AltCLIP ---
        candidate_texts = []
        for temp_idx in top_k_indices[0]:
            temp_tokens = torch.cat([tokens_generated or tokens, temp_idx.unsqueeze(0)], dim=-1)
            temp_sentence = tokenizer.decode(temp_tokens[0].tolist()[-clip_text_max_len:])
            candidate_texts.append(temp_sentence)

        with torch.no_grad():
            text_inputs = clip_processor(text=candidate_texts, return_tensors="pt", padding=True).to(device)
            text_features = clip_model.get_text_features(**text_inputs)
            text_features = F.normalize(text_features, dim=-1)
            image_sentence_score = (images_feature @ text_features.T).squeeze(0)

        # --- select token guided by similarity ---
        best_idx = torch.argmax(beta * top_k_probs[0] + alpha * image_sentence_score)
        selected_token = top_k_indices[0, best_idx].unsqueeze(0).unsqueeze(0)

        # update sequence
        tokens = torch.cat([tokens, selected_token], dim=-1)
        if tokens_generated is None:
            tokens_generated = selected_token
        else:
            tokens_generated = torch.cat([tokens_generated, selected_token], dim=-1)

        # stop if EOS
        if b == 1 and selected_token.item() == eos:
            sentence = tokenizer.decode(tokens.squeeze(0).tolist())
            return sentence

        # forward next token
        new_emb = model.transformer.wte(selected_token)
        outputs = model(
        ...
        next_token_logits = outputs.logits[:, -1, :]
        past_key_values = outputs.past_key_values
        past_hidden_states = torch.cat([past_hidden_states, outputs.hidden_states[-1]], dim=1)
    ...
    if b == 1:
        ...
        result = []
        for temp_tokens in tokens:
            new_tokens = temp_tokens.tolist()
            if eos in new_tokens:
                new_tokens = new_tokens[:new_tokens.index(eos) + 1]
            result.append(tokenizer.decode(new_tokens))
        return result
...
def image_sentence_similarity(
    ...
    # (b, n_seq) + (b, top_k) => (b*k, n_seq + 1)
    if tokens_generated is None:
        temp_tokens = top_k_indices.view(-1).unsqueeze(1)
    else:
        b, n = tokens_generated.size()
        tokens_generated = tokens_generated.unsqueeze(1).expand(-1, top_k, -1).reshape(b * top_k, n)
        top_k_indices = top_k_indices.view(-1).unsqueeze(1)
        temp_tokens = torch.cat([tokens_generated, top_k_indices], dim=1)

    # Convert tokens → câu (text)
    sentences = []
    for temp_token in temp_tokens:
        sentence = tokenizer.decode(temp_token[-clip_text_max_len:].to('cpu').tolist())
        sentences.append(sentence)

    # Encode text bằng SentenceTransformer
    text_features = clip_model.encode(
        sentences,
    ...
    text_features = text_features.view(-1, top_k, text_features.size(-1))  # (b, k, hidden)
    image_features = F.normalize(images_feature.float(), dim=-1).unsqueeze(1)  # (b, 1, hidden)

    # Cosine similarity
    score = torch.matmul(text_features, image_features.transpose(1, 2)).squeeze(-1)  # (b, k)
